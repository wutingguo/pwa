...
const getOutput = (devMode, project) => {
...
const getPluginConfig = (devMode, isCN, project, isClientProject, isMobileProject) => {
  const pwaSourcePath = path.resolve(
    __dirname,
    `../../resource/${isClientProject ? 'pwa-client' : 'pwa'}`
  );
...
const getCdnUrl = (project, isMobileProject) => {
  const needCdnClients = ['gallery-client', 'gallery-client-mobile', 'slide-show-client', 'slide-show-client-mobile', ]
  let appUrl = 'gallery-client';
  const index = project.indexOf('-client');
  if (index !== -1) {
    appUrl = project.substring(0, index + 7);
  }
  return `https://d37m7wdo4ej41d.cloudfront.net/${isMobileProject ? 'wap' : 'pc'}/${appUrl}/`
}

// https://d37m7wdo4ej41d.cloudfront.net/wap/slide-show-client/
const getBaseConfig = ({ devMode, isCN, project, isClientProject, isMobileProject }) => {
  const outputPath = getOutput(devMode, project, isClientProject);

  // const publicPath = isMobileProject
  //   ? 'https://d37m7wdo4ej41d.cloudfront.net/wap/gallery-client/'
  //   : 'https://d37m7wdo4ej41d.cloudfront.net/pc/gallery-client/';
  
  const publicPath = getCdnUrl(project, isMobileProject);
