...
export default class SignUp extends XPureComponent {
  ...
  async onSubmit(e) {
    ...
    if (needVerificationCode) {
      this.sendSignUpRequest('', requestData);
    } else {
      this.executeCaptcha(requestData);
    }
  }

  sendSignUpRequest(token = '', requestData) {
    const { urls } = this.props;
    this.setState({ isLoading: true });
    // 将谷歌返回的token塞进请求对象里
    requestData.reCAPTCHAToken = token;
    this.setState({ isLoading: true });
  ...
  executeCaptcha(requestData) {
    if (!window.grecaptcha) {
      return this.executeCaptcha(requestData);
    }
    const that = this;
    // 谷歌接入验证该用户是否为机器人
    debugger
    window.grecaptcha.ready(function () {
      window.grecaptcha
        .execute('6LdrrCYaAAAAAJipRNIiLsgCuz7yDsG5DfrBoYDl', {
          action: 'register'
        })
        .then(function (token) {
          // Add your logic to submit to your backend server here.
          that.sendSignUpRequest(token, requestData);
        })
        .catch(e => {
          // 发生错误之后显示验证码
          that.setState({
  ...
  onLoginSuccess() {
    ...
    boundGlobalActions.getUserInfo().then(() => {
      history.replace('/software/success?registered=true');
      ...
      <div className="sign-up-form-container">
        ...
        >
          ...
          <div className="switch-to-signin">
            ...
            <span><Link to="/software/sign-in">{t('FORM_SIGN_IN_ARROW')}</Link></span>
          </div>
        </form>
        <XLoading
          isShown={isLoading}
