...
import { getPlanList, getPlanSummary, getOrderConfirm } from '@resource/pwa/services/subscription';
...
const formatPlanData = ({
  ...
  const plans = products.map(m => {
    ...
    const currentPrice = priceInfos.find(v => v.cycle === usedCycleId && v.level === usedLevelId);
    const currentLevel = selectLevelData.find(v => v.id === usedLevelId);
    const currentCycle = selectCycleData.find(v => v.id === usedCycleId);
...
const getPlanConfirm = that => {
  ...
  getOrderConfirm({ subscribe_orders }, galleryBaseUrl)
    .then(result => {
      const { card_token, product_list } = result;
      that.setState({
        isShowConfirmPage: true,
        orderConfirmInfo: {
          card_token,
          product_list
        }
