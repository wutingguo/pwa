...
export default class ConfirmPage extends Component {
  getMessage = item => {
    ...
    if (upgrade && adjust_cycle) {
      return [
        <div>{t('CONFIRM_SUBSCRIPTION_UPGRADE', { cost: displayPrice(currency, upgrade_cost) })}</div>,
        <div style={{height: 20}}/>,
        <div>{t('CONFIRM_SUBSCRIPTION_ADJUST_CYCLE', { cycle: cycle_name, cost: displayPrice(currency, cycle_cost) })}</div>
      ]
    }

    // case2:【升级 + 不调周期】
    if (upgrade && !adjust_cycle) {
      return <div>{t('CONFIRM_SUBSCRIPTION_UPGRADE', { cost: displayPrice(currency, upgrade_cost) })}</div>
    }
    ...
    // case6:【只调周期】
    if (adjust_cycle) {
      return <div>{t('CONFIRM_SUBSCRIPTION_ADJUST_CYCLE', { cycle: cycle_name, cost: displayPrice(currency, cycle_cost) })}</div>;
    }
