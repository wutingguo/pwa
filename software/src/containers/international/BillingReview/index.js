...
export default class BillingReview extends Component {
  ...
  getDesignServiceOrderDetails = async () => {
    ...
    const serviceOrderId = designService.get("serviceOrderId");
    if (serviceOrderId <= 0) {
      const serviceOrderId = this.urlObj.service_order_id;
  ...
  async onSubmit() {
    ...
    try {
      ...
      data = data || {};
      if (isRequestSuccess) {
        history.replace(`/software/ds-order-confirm?service_order_id=${this.urlObj.service_order_id}`);
      } else {
        isSubmiting = false;
  ...
  handleBack = () => {
    ...
    history.push(`/software/ds-order-form`);
    return true;
  }
  ...
  render() {
    ...
    if (hasHistoryCredit) { // 判断有没有绑定过信用卡，条件待补充
