...
import { pageSizeOptions } from './opts';

import './index.scss';

class XPagePagination extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      editedPageSize: props.currentPageSize || 24, // 新增页数
    };
  }
  ...
  goToPage() {
    const { editedPage, editedPageSize } = this.state;
    this.changePageIndex(editedPage, editedPageSize);
  }

  changePageIndex(pageIndex, pageSize = 24) {
  ...
  turnPrevPage() {
    ...
    const { editedPageSize } = this.state;
    this.changePageIndex(currentPage - 1, editedPageSize);
  }
  ...
  changeToTargetPage(itemData) {
    ...
    const { editedPageSize } = this.state;
    this.changePageIndex(itemData.pageIndex, editedPageSize);
  }
  onKeyUp(e) {
    // 回车键
    if (e.keyCode == 13) {
  ...
  changePageSize({ value }) {
    this.setState({ editedPageSize: value, editedPage: 1 }, () => {
      this.goToPage();
    });
  }

  render() {
    const { totalPage, currentPage, isShowPageSize = false } = this.props;
    const { editedPage, editedPageSize } = this.state;
      ...
      <div className="page-pagination">
        ...
        {isShowPageSize && (
          <XSelect
            ...
            style={{ width: __isCN__ ? 80 : 100 }}
            onChanged={this.changePageSize}
          />
        )}
...
XPagePagination.propTypes = {};

XPagePagination.defaultProps = {
  totalPage: 8,
  currentPage: 8,
};

export default XPagePagination;
