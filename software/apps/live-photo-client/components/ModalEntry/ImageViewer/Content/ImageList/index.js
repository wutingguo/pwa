...
export default function ImageList(props) {
  ...
  // 选中自动计算移动距离
  function autoMove() {
    const index = photoList.findIndex(photo => {
      return photo.show_enc_content_id === point.show_enc_content_id;
    });
    const { width } = listRef.current.getBoundingClientRect();
    const b = -boxW,
      r = width >> 1;
  ...
  const isDisabled = useMemo(() => {
    const { width = 0 } = listRef.current?.getBoundingClientRect() || {};
    const count = photoList.length;
    const maxWidth = -count * boxW + width;
    if (moveX === 0) {
      return 'left';
  ...
  return (
    <Container className={className}>
      ...
      >
        <IconLeft fill={isDisabled === 'left' ? '#fff' : undefined} />
      </PointerLeft>
      ...
      >
        <IconLeft fill={isDisabled === 'right' ? '#fff' : undefined} />
      </PointerRight>
    </Container>
  );
}
