...
const onJump = (e, history) => {
    ...
    window.logEvent.addPageEvent({
        name: 'DSOrderConfirm_Click_My_Orders'
    });
    history.push(`/software/designer/my-orders`);
    e.preventdefault();
}
...
export default class Designer extends Component {
    ...
    componentDidMount() {
        ...
        boundProjectActions.getConfirmInfo({ saasBaseUrl, serviceOrderId: this.urlObj.service_order_id })
            .then(({ ret_code, data }) => {
                if (ret_code == 200000) {
                    this.setState(data);
                }
