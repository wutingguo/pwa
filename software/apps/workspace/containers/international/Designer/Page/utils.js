...
export const format = ({ state, projectId }) => {
  let {
    coverArr,
    productConfirmat,
    currPage,
  ...
  favoriteArr = favoriteArr
    ...
    .map(({ src, desc, orientation }) => ({ src, desc, orientation }));
  coverArr = coverDesign.specifyCoverPhoto.checked
    ? coverArr
        .filter(img => img.active)
        .map(({ src, desc, orientation }) => ({ src, desc, orientation }))
    : [];
  coverDesign = Object.values(coverDesign)
    .filter(s => s.checked)
    .map(({ text, extra = '' }) => `${text} ${extra}`);
  liningDesign = Object.values(liningDesign)
    .filter(s => s.checked)
    .map(({ text, extra = '' }) => `${text} ${extra}`);
  edgeDesign = edgeDesign.filter(s => s.checked)[0]?.value;
  imageSelection = imageSelection.filter(s => s.checked)[0]?.value;
  return {
