import { getAddressDetail, getAddressDetailByOrder } from '../../utils/service';
...
const handleSelectAddress = (that, address) => {
  ...
  const { province, city, area, street, streets } = address;
  that.setState({
    streets,
...
const getAddressDetailData = that => {
  ...
  const fetchAddress = address_id ? getAddressDetail.bind(undefined, address_id) : getAddressDetailByOrder.bind(undefined, order_number);
...
const onCreate = that => {
  ...
  const { a_type, address_id, isShowLoading, warnText, order_number, streets, ...rest } = that.state;
