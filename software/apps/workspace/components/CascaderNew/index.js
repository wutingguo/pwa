...
class Cascader extends React.Component {
  constructor(props) {
    ...
    this.getSubArea = (type, uidpk, level) =>
      handler.getSubArea(this, type, uidpk, level);
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(oldProvince !== newProvince || oldCity !== newCity || oldArea !== newArea || oldStreet !== newStreet) {
      this.setState({
  ...
  render() {
    ...
    // 拼接显示的地址
    const address = `${province}/${city}/${area}/${street}`.replace(
      /(\/{2,}|\/$)/g,
      ''
    );
...
Cascader.propTypes = {
  isSupportStreet: PropTypes.boolean,
