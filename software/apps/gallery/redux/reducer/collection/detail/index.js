...
const detail = (state = fromJS({}), action) => {
  switch (action.type) {
    ...
    case API_SUCCESS:
      switch (action.apiPattern.name) {
        case SAAS_GET_COLLECTION_DETAIL: {
          ...
          return state.merge(fromJS(newItem));
        }
        ...
        case SAAS_GALLERY_STATUS_POLLING: {
          ...
          if (data && Array.isArray(data)) {
            data.forEach(item => {
              statusMap[item.enc_image_id] = item;
            });
          }
        ...
        case GET_COLLECTION_GROUP_INFO: {
          const data = convertObjIn(get(action.response, 'data'));
          return state.merge({
            collection_group_info: fromJS(data),
          });
        }
        case COUNT_FRESH_IMAGES: {
          const data = get(action.response, 'data');
          return state.merge({
            count_fresh_images: fromJS(data),
          });
        }
        case GET_SET_VIDEO_BY_SET: {
          const data = get(action.response, 'data') || {};
          return state.update('gallery_video_info', () => {
            return fromJS({
              ...data,
        ...
        case SAAS_CLIENT_SLIDESHOW_PREVIEW_GET_DETAIL: {
          const data = get(action.response, 'data');
          if (!data) {
            return state.updateIn(['gallery_video_info', 'showLoding'], () => false);
          }
          return state.update('gallery_video_info', gallery_video_info => {
            return fromJS({
              ...gallery_video_info.toJS(),
    ...
    case UPDATE_SELECTED_IAMGE:
      ...
      return state.update('images', images =>
        images.map(s => {
          if (selectedIds.includes(s.get('enc_image_uid') || s.get('image_uid'))) {
    ...
    case SELECTION_VIDEO: {
      const isSelectVideo = state.getIn(['gallery_video_info', 'isSelectVideo']);
      return state.updateIn(['gallery_video_info', 'isSelectVideo'], () => !isSelectVideo);
    }
