...
const ChooseImgModal = props => {
  ...
  return (
    ...
    >
      ...
      />
      <div className="right">
        <div className={classNames('button', { selected: 'selected' })} onClick={goShopCart}>
          Continue
        </div>
        {
          <div
            className={shake ? 'select_tips shake' : 'select_tips'}
          >{`${allSelectedImgsNum} photo(s) selected`}</div>
        }
      </div>

      <div className="title">Select Photos</div>
      ...
      {curImages.length > 0 ? (
        ...
        <div className="no_photo">No Photos</div>
      )}
    </XModal>
  );
};
