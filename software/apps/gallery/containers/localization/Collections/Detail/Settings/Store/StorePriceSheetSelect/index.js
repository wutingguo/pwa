...
const StorePriceSheetSelect = ({
  ...
  const getSelectOptions = useCallback(async () => {
    ...
    try {
      const { options, selectedOption, priceSheetList } =
  ...
  const handleSelectPriceSheet = useCallback(
    async option => {
      ...
      if (presetState) {
        instantUpdate('rack_id', option.value);
  ...
  let { options, selectedOption } = state;
  if (presetState && options) {
    selectedOption = options.find(item => item.value === curRackIdInPreset);
  }
