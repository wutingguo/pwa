...
const handleCreate = that => {
  ...
  const confirmData = {
    ...
    close: hideConfirm,
    title: '',
    message: t('SLIDESHOW_UPGRADE_TIP'),
    buttons: [
      {
        className: 'pwa-btn',
        text: t('SLIDESHOW_UPGRADE_BTN'),
        onClick: that.toCheckoutPlan
      }
    ]
  };
...
const handleDelete = (that, item, cb) => {
  ...
  const data = {
    ...
    close: hideConfirm,
    title: t('SLIDESHOW_DELETE'),
    message: (
      <div style={{ textIndent: 10, textAlign: 'center' }}>
        {t('DELETE_SLIDESHOW')}
        {t('DELETE_SLIDESHOW_TIP')}
      </div>
    ),
    buttons: [
      {
...
const handleDownloadAndShare = (that, item, shareSlideshowTabKey) => {
  ...
  Promise.all([
    ...
    console.log('handleDownloadAndShare--responses: ', responses);
