  ...
  render() {
    ...
    return (
      ...
      >
          ...
          <div className="sets-wrap">
            {
              ...
              (
                otherSets.map(set => {
                  return <span key={set.get('set_uid')} className={`set-item ellipsis ${+setUid === +set.get('set_uid') ? 'actived' : ''}`} onClick={() => this.onSelectSet(set.get('set_uid'))}>{set.get('set_name')}</span>
                })
              ) : null
            }
          </div>
        ...
        {
          showTip ? <span className="tips-content">{t('IMAGE_COPY_MOVE_CHANGE_SET_TIP')}</span> : null
        }
