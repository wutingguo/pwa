...
class EmailEnter extends XPureComponent {
  ...
  render() {
    ...
    const setting_details =
      (login_information_config && login_information_config.get('setting_details')) || [];
    const isNeedLoginInfo =
      login_information_config && login_information_config.get('collect_type') === 1;
    const Email = setting_details.find(item => item.get('information_id') === 2)?.toJS() || {};
    const Name = setting_details.find(item => item.get('information_id') === 3)?.toJS() || {};
    const Other = setting_details.find(item => item.get('information_id') === 4)?.toJS() || {};
    ...
    const inputProps = {
      ...
      tipContent,
      suffixIcon: (
        <XIcon type="input-clear" iconWidth={18} iconHeight={18} onClick={this.onInputClear} />
      ),
    };
    ...
    const passwordProps = Object.assign({}, inputProps, {
      ...
      suffixIcon: (
        <XIcon type="input-clear" iconWidth={18} iconHeight={18} onClick={this.onPasswordClear} />
      ),
    });
