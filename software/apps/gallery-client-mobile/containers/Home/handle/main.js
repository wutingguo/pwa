...
import { get, merge, unionBy } from 'lodash';
...
import { CONFIRM_MODAL, CONFIRM_PAY_MODAL } from '@resource/lib/constants/modalTypes';
...
} from '@apps/gallery-client-mobile/constants/modalTypes';
import {
  getEmailCacheKey,
  getGuestUidCacheKey,
  getPhoneCacheKey,
...
import { miniCodeModalConfig } from '../modalConfig';
...
const markOrUnmarkFavorite = (that, item, reRender, modalProps) => {
  const { boundProjectActions, boundGlobalActions, sets, favorite, guest, collectionId } =
    that.props;
  let { currentSetIndex, favoriteImgIds, images } = that.state;
...
const showFavoriteList = (that, submitStatus) => {
  ...
  const { favoriteImgIds, currentSetIndex, viewImageIndex, images: imgs } = that.state;
  ...
  const modalProps = {
    ...
    unmark: (item, reRender) => {
      if (!that.verifySubmit(true)) {
        return;
      }
      return markOrUnmarkFavorite(that, item, reRender, modalProps);
    },
    downloadGallery: params => {
      that.downloadGallery({ ...params, submitStatus });
    },
  };
  boundGlobalActions.showModal(VIEW_FAVORITE_LIST_MODAL, modalProps);
};
...
const downloadGallery = async (that, params) => {
  ...
  const { showModal, hideModal, addNotification, showConfirm, hideConfirm } = boundGlobalActions;

  // 微信不支持下载
  if (/micromessenger/i.test(navigator.userAgent)) {
    showConfirm({
      message: (
        <div style={{ fontSize: 28, lineHeight: '48px' }}>
          {t('UNSUPPORT_DOWNLOAD_COLLECTION_WEIXIN')}
        </div>
      ),
      close: () => hideConfirm(),
      buttons: [
        {
          text: t('UNSUPPORT_DOWNLOAD_COLLECTION_WEIXIN_BTN'),
          className: 'pwa-btn-wap',
          onClick: () => {
            ...
            inputElement.remove();
            addNotification({
              message: t('UNSUPPORT_DOWNLOAD_COLLECTION_WEIXIN_BTN_1'),
              level: 'success',
              autoDismiss: 2,
            });
            hideConfirm();
  ...
  if (isDownloadCollection) {
    ...
    if (!isNeedCheckoutPin) {
      if (!__isCN__ && !isSelectPhotoed && download_limited && !isHasExistedZip) {
        boundGlobalActions.showModal('DOWNLOAD_CHOOSE_IMG_MODAL', {
          ...that.props,
          urls: urls.toJS(),
          downloadSelectPhoto: true,
...
const onInitFace = that => {
  ...
  boundProjectActions.getFaceImgs().then(res => {
    ...
    const faceImgs = res.data?.group_infos || [];
    const selectCache =
...
const handleOrderPay = (that, config, callbackFn) => {
  const {
    boundGlobalActions,
    urls,
    favoriteImageList,
  ...
  if (config.order_type === galleryOrderType.weixin) {
    if (getQs('sourceFrom') === 'mp') {
      // 微信小程序环境直接打开订单详情
      const query = `guest_uid=${guest.get(
        'guest_uid'
      )}&favorite_id=${favorite_id}&collection_uid=${encodeURIComponent(collectionUid)}`;
      wx.miniProgram.navigateTo({
        url: `/pages/orderDetail/index?${query}`,
      });
    } else {
  ...
  } else if (config.order_type === galleryOrderType.offline) {
...
const getOrderStatus = async (that, params) => {
  ...
  // list订单列表第一个是最新的有效订单， 并且order_status === 0 || 1 待支付才调起继续支付弹窗
...
const confrimPayTypeModal = (that, textObj, configs) => {
  ...
  const { boundGlobalActions, urls, favorite, guest, collectionUid } = that.props;
  const { showModal, hideModal } = boundGlobalActions;
  const galleryBaseUrl = urls.get('galleryBaseUrl');
  const { title, message, btnText1, btnText2 } = textObj;
  const offlineBtnShow =
    payTypeConfigs &&
    payTypeConfigs.find(item => item.pay_type === galleryOrderType.offline).pay_switch;
  const offlineBtn = {
    text: '线下支付',
    icon: offline,
    className: 'payBtn',
...
const continuePaymentModal = that => {
  ...
  const { showModal, hideModal } = boundGlobalActions;
  const galleryBaseUrl = urls.get('galleryBaseUrl');
  const callbackFn = () => continuePaymentModal(that);
  showModal(CONFIRM_MODAL, {
    title: '',
    message: '\n \n 当前存在待支付订单，是否继续支付？\n \n ',
    btnOpenClose: true,
    ...
    style: { width: '600px' },
    isHideIcon: true,
    // close: () => that.cancelOrder({galleryBaseUrl, order_number: that.state.orderConfig?.order_number, hideModal}),
    // close: () => { hideModal(CONFIRM_MODAL) },
    buttons: [
      {
        text: '重新选片',
