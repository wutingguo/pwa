...
const markOrUnmark = enc_image_uid => {
  return (dispatch, getState) => {
    ...
    const hasMarked =
      favoriteImageList && favoriteImageList.find(m => m.get('enc_image_uid') === enc_image_uid);
    if (hasMarked) {
      return dispatch(unmarkFromFavorite(enc_image_uid, favorite.get('favorite_uid')));
    }
