...
class BindEmailModal extends Component {
  ...
  render() {
    ...
    const message =
      data.get('message') ||
      t(`收藏您喜欢的照片前，请先完成登录，以下信息将用于摄影师记录您的选片反馈`);
    const close = data.get('close');
    const wrapClass = classNames('bind-email-modal-wrap-cn', data.get('className'));
    const login_information_config =
      collectionSetting && collectionSetting.get('login_information_config');
    const setting_details =
      login_information_config && login_information_config.get('setting_details');
    const Email =
      (setting_details && setting_details.find(item => item.get('information_id') === 2).toJS()) ||
      {};
    const Name =
      (setting_details && setting_details.find(item => item.get('information_id') === 3).toJS()) ||
      {};
    const Other =
      (setting_details && setting_details.find(item => item.get('information_id') === 4).toJS()) ||
      {};
    const inputProps = {
      className: 'create',
      value: inputValue,
