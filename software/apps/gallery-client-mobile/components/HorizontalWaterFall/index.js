...
const H = ({ renderCard, list = fromJS([]), designSetting }) => {
  ...
  const minHeight = (thumbnail_size || '').toLowerCase() === 'large' ? 400 : 254;
  const gapWidth = (grid_spacing || '').toLowerCase() === 'large' ? 16 : 6;
  const fn = () => {
    ...
    tempList.forEach(item => {
      // item.set('ratio') = item.get('width') / item.get('height');
        ...
        {list.map((item, idx) => {
          return (
            <div
              ...
              style={
                (thumbnail_size || '').toLowerCase() === 'large'
                  ? {
                      width: '100%',
                    }
                  : {
