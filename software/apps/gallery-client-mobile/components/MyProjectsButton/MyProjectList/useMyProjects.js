...
const useMyProjects = ({ boundGlobalActions }) => {
  ...
  const buildEditorUrl = useCallback(
    async ({ projectId, parentCategoryCode }) => {
      ...
      const href = `${estoreBaseUrl}prod-assets/app/cxeditor/index.html?supplierId=${supplier_id}&entityId=${supplierCode}&rackId=${rackId}&rackSpuId=${rackSpuId}&categoryCode=${parentCategoryCode}&storeId=${storeId}&collectionUid=${collectionUid}&collectionId=${collectionId}&estoreSource=1&virtualProjectGuid=${projectInfo.guid
        }&packageType=single&initGuid=${projectInfo.guid
        }&estoreFrom=MY_PROJECT&backHref=${encodeURIComponent(location.href)}&esTK=${localStorage.getItem('_tk_') || ''
        }`;
